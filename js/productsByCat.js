...
async function productsByCat(){
    ...
    const categoriaResponse = await cache.match('/categoria');

    categoria = await categoriaResponse.json()

    await axios({
        method: 'GET',
        url: `https://intellistock-api.herokuapp.com/products/company/${user.id}/category/${categoria.id}`
    }).then(async (response) => {
        ...
        `
        ${response.data.name}
        <button class=" btn btn-primary btn-add" onclick="addProdutos()" style="float: right;">Adicionar</button>
        `
        
        var tbody = document.getElementById('conteudo-produtos-cat');
